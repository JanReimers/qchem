...
import qchem.AtomCalculation;        // AtomCalculation, AtomType, Model, SpinGroup (atomic exponent-pool DFT)
...
static Pool PoolFor(AtomType type, SpinGroup pol, int Z)
{
    if (type==AtomType::Gaussian)
        return (pol==SpinGroup::Polarized) ? Pool{20, 0.01, 4000.0*Z} : Pool{20, 0.05, 10000.0*Z};
    return Pool{slater_N(Z), 0.31, 3.0*Z};   // Slater (pol- and model-independent)
}

struct AtomDFTCase { AtomType type; int Z; SpinGroup spin; Model model; double tol; };   // exponent-pool atom DFT
struct PGCase      { int Z; SpinGroup spin; double tol; };                               // molecular basis on an atom
...
TEST_P(A_DFT_atom, Energy)
{
    ...
    const Pool p = PoolFor(c.type, c.spin, c.Z);
    AtomCalculation calc(c.Z, 0, {.type = c.type, .N = p.N, .emin = p.emin, .emax = p.emax,
                                  .model = c.model, .spin = c.spin, .xalpha = pt.GetSlaterAlpha(c.Z)},
                         dft_scf_params(c.Z));
...
INSTANTIATE_TEST_SUITE_P(SG_U,  A_DFT_atom, ::testing::ValuesIn(WithZ({2,4,10,18,36,54}, AtomDFTCase{AtomType::Gaussian,0,SpinGroup::UnPolarized,Model::Xalpha,2e-3})),  ZName<AtomDFTCase>);
INSTANTIATE_TEST_SUITE_P(SL_U,  A_DFT_atom, ::testing::ValuesIn(WithZ({2,4,10,18,36,54}, AtomDFTCase{AtomType::Slater,  0,SpinGroup::UnPolarized,Model::Xalpha,2e-3})),  ZName<AtomDFTCase>);
INSTANTIATE_TEST_SUITE_P(LSDA,  A_DFT_atom, ::testing::ValuesIn(WithZ({2,10,18,36},      AtomDFTCase{AtomType::Slater,  0,SpinGroup::UnPolarized,Model::LDA,   2.5e-3})), ZName<AtomDFTCase>);
INSTANTIATE_TEST_SUITE_P(SG_P,  A_DFT_atom, ::testing::ValuesIn(WithZ({1,3,7,37,53},     AtomDFTCase{AtomType::Gaussian,0,SpinGroup::Polarized,  Model::Xalpha,1e-3})),  ZName<AtomDFTCase>);
INSTANTIATE_TEST_SUITE_P(SL_P,  A_DFT_atom, ::testing::ValuesIn(WithZ({1,3,7,37,53},     AtomDFTCase{AtomType::Slater,  0,SpinGroup::Polarized,  Model::Xalpha,2e-3})),  ZName<AtomDFTCase>);
...
TEST_P(A_PG_DFT, Energy)
{
    ...
    Calculation calc(atom, {.basis = "dzvp", .model = Model::Xalpha, .spin = c.spin,
...
INSTANTIATE_TEST_SUITE_P(U, A_PG_DFT, ::testing::ValuesIn(WithZ({2,4,10,18,36}, PGCase{0,SpinGroup::UnPolarized,3e-3})),   ZName<PGCase>);   // Ar(18) ~2.94e-3 vs NIST
INSTANTIATE_TEST_SUITE_P(P, A_PG_DFT, ::testing::ValuesIn(WithZ({3,5,11,37},   PGCase{0,SpinGroup::Polarized,  5.1e-3})), ZName<PGCase>);   // Z=51 slow -> omitted
