...
class SHF_U : public virtual QchemTester
{
    virtual Hamiltonian* GetHamiltonian(cl_t& cluster) const
    {
        ...
        return new Ham_SHF_U(cluster,alpha_ex,GetMeshParams(),itsBasisSet);
    }
};
...
, public TestAtom, SG_OBasis, SHF_U, TestUnPolarized
{
public:
...
, public TestAtom, SL_OBasis, SHF_U, TestUnPolarized
{
public:
...
class A_PG_SHF_U : public ::testing::TestWithParam<int>
, public TestMolecule, PG_OBasis, SHF_U, TestUnPolarized
{
public:
    A_PG_SHF_U() : TestMolecule() {};
    void Init()
    { 
...
TEST_P(A_PG_SHF_U,Multiple)
{
    Init();
    Iterate({40,1e-3,1.0,0.0,false});
    EXPECT_LT(RelativeDFTError(),MaxRelErrE);
}
INSTANTIATE_TEST_CASE_P(Multiple,A_PG_SHF_U,::testing::Values(2,4,10,18,36));


//
//  Polarized tests.
//
class SHF_P : public virtual QchemTester
{
    virtual Hamiltonian* GetHamiltonian(cl_t& cluster) const
    {
        double alpha_ex=QchemTester::itsPT.GetSlaterAlpha(GetZ());
        return new Ham_SHF_P(cluster,alpha_ex,GetMeshParams(),itsBasisSet);
    }
};

class A_SG_SHF_P : public ::testing::TestWithParam<int>
, public TestAtom, SG_OBasis, SHF_P,TestPolarized
{
public:
    A_SG_SHF_P() : TestAtom(GetParam()) {};
    void Init(int N, double emin, double emax, int LMax)
    {
        SG_OBasis::Init(N,emin,emax,LMax);
        QchemTester::Init(1e-3);
...
, public TestAtom, SL_OBasis, SHF_P, TestPolarized
{
public:
    A_SL_SHF_P() : TestAtom(GetParam()) {};
    void Init(int N, double emin, double emax, int LMax)
    {
        SL_OBasis::Init(N,emin,emax,LMax);
        QchemTester::Init(1e-3);
...
class A_PG_SHF_P : public ::testing::TestWithParam<int>
, public TestMolecule, PG_OBasis, SHF_P, TestPolarized
{
public:
    A_PG_SHF_P() : TestMolecule() {};
    void Init()
    { 
        Molecule* m=new Molecule;
        m->Insert(new Atom(GetParam(),0.0,Vector3D<double>(0,0,0)));
