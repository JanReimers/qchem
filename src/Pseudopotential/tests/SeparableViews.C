...
// Projector(q) (plane-wave) and the real leaf RadialR(r) (molecular/atomic).  They MUST be the same function,
                              ...
                              const qcMesh::RadialMesh& mesh)
{
    ...
    for (size_t i=0;i<R.size();i++)
        s += W[i] * sep.RadialR(Z,p,R[i]) * std::sph_bessel((unsigned)l, q*R[i]);
    return s;
}
...
TEST(SeparablePotentialViews, RealBesselTransformMatchesReciprocal)
{
    ...
    for (size_t p=0;p<sep.Count(Z);++p)
    {
        ...
        for (double q : {0.5, 1.0, 2.0, 3.0, 5.0})
        {
            ...
            double rhs = sep.RadialQ(Z, p, q) / s4pi;            // reciprocal view: Projector/sqrt(4pi)
...
TEST(SeparablePotentialViews, CesiumFChannelBesselTransformMatchesReciprocal)
{
    ...
    const HGH_SeparablePotential& sep = pp.nonlocal;
    int maxl=0; for (size_t p=0;p<sep.Count(Z);++p) maxl=std::max(maxl, sep.L(Z,p));
    ...
    for (size_t p=0;p<sep.Count(Z);++p)
    {
        int l = sep.L(Z,p);
        for (double q : {0.5, 1.0, 2.0, 3.0, 5.0})
            EXPECT_NEAR(BesselTransform(sep, Z, p, l, q, mesh), sep.RadialQ(Z, p, q)/s4pi,
                        1e-3*std::fabs(sep.RadialQ(Z,p,q)/s4pi) + 1e-5)
...
TEST(SeparablePotentialViews, AllChannelsBesselTransformMatchesReciprocal)
{
    ...
    qcMesh::RadialMesh mesh = qcMesh::MakeRadial({.radial=qcMesh::RadialKind::Log, .nRadial=4000,
                                                  .logStart=1e-4, .logStop=12.0});
    const double s4pi = std::sqrt(4.0*Pi);
    ASSERT_EQ(sep.Count(Z), 7u);               // 3 + 2 + 2
    for (size_t p=0;p<sep.Count(Z);++p)
    {
        int l = sep.L(Z,p);
        for (double q : {0.5, 1.0, 2.0, 3.0, 5.0})
        {
            ...
            double rhs = sep.RadialQ(Z, p, q) / s4pi;
            EXPECT_NEAR(lhs, rhs, 1e-3*std::fabs(rhs) + 1e-5)
