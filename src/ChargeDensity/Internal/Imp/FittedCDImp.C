...
import qchem.Blaze;
import qchem.Math;
import qchem.ScalarFunction;             // ScalarFunction<double> (FittedCD IS-A one)
...
//-----------------------------------------------------------------------------
//
//  DoFit:  every finite density presents its OWN ProjectedDensity_AO -- a matrix-backed density (IrrepCD/
//  CompositeCD) yields the exact <rho|c>; a matrix-free SAD seed (NumericCD) overlap-fits its own rho(r).
//  FittedCD is agnostic to which and knows nothing of seeding.
//
template <class T> void FittedCDImp<T>::DoFit(const rChargeDensity& cd)
{
    ...
    assert(ao && "FittedCD::DoFit: every finite density must present its own ProjectedDensity_AO");
    itsFitter->DoFit(*ao);
}
