...
//  And C nd e are then calculated for various matrices H.  We need to find a decomposition
...
public:
    ...
    virtual void    SetBasisOverlap(const smat_t<T>& S)=0;
    virtual rvec_t  Get_BS_Diagonal() const=0;
    virtual Ud_t    Solve(const smat_t<T>& H) const=0; //H=Hamiltonian/Fock matrix.  H'=
