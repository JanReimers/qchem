...
module;
#include <iosfwd>
#include <map>       // std::map<int,hmat_t<T>> (MakeSeparablePotentialByL, the per-l KB diagnostic)
#include <memory>
#include <string>
#include <vector>
...
template <class T> class tGPW_IBS
    ...
    , public Pseudopotential::Integrals_Pseudo<T>   // external-PP assembly (real-space); PW_Pseudo casts ACROSS to this
...
public:
    ...
    virtual hmat_t<T> MakeLocalPotential   (const Structure* cl, const Pseudopotential::LocalPotential&     loc) const override;
    ...
    virtual hmat_t<T> MakeLocalPotentialLong (const Structure* cl, const Pseudopotential::LocalPotential& loc) const override;
    virtual hmat_t<T> MakeLocalPotentialShort(const Structure* cl, const Pseudopotential::LocalPotential& loc) const override;
    virtual hmat_t<T> MakeSeparablePotential(const Structure* cl, const Pseudopotential::SeparablePotential& nl ) const override;
    virtual std::map<int,hmat_t<T>> MakeSeparablePotentialByL(const Structure* cl, const Pseudopotential::SeparablePotential& nl ) const override;
