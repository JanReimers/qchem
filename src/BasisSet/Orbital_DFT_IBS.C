...
public:
    ...
    virtual const ERI3<T>& Overlap3C  (const FIT_SF_ABS& c) const;
    ...
    virtual const ERI3<T>& Repulsion3C(const FIT_CD_ABS& c) const;

    virtual FIT_CD_ABS* CreateCDFitBasisSet (const Structure*, const qcMesh::MeshParams&) const=0;
    virtual FIT_SF_ABS* CreateVxcFitBasisSet(const Structure*, const qcMesh::MeshParams&) const=0;
    virtual vec_t<T> Overlap3C  (const smat_t<T>& Dcd, const FIT_SF_ABS* c) const;
    virtual vec_t<T> Repulsion3C(const smat_t<T>& Dcd, const FIT_CD_ABS* c) const;
protected:
    virtual ERI3<T> MakeOverlap3C  (const FIT_SF_ABS& c) const=0;
    virtual ERI3<T> MakeRepulsion3C(const FIT_CD_ABS& c) const=0;
};
