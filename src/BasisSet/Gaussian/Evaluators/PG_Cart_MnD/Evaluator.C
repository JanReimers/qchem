// File: BasisSet/Gaussian/Evaluators/PG_Cart_MnD/Evaluator.C
//
...
export namespace qchem::BasisSet::Gaussian::Evaluators::PG_Cart_MnD
{
...
public:
    ...
    // Gaussian::LatticeSum1E (on the host IBS).  (Rs,phases) is a {R}+{e^{ik.R}} weighted point set -- FUTURE:
    ...
    //! The Bloch phase of an integer cell offset (== Gaussian::LatticeSum1E::cellphase_t -- the same
    ...
    // Gaussian::LatticeSum1E: the analytic 3-centre (Overlap3C) MATRIX of a per-atom LOCAL Gaussian operator --
    ...
    chmat_t MakeLocalGaussian(const cellphase_t& phase, const UnitCell& A, const Structure* cl,
                              const std::function<Gaussian::LatticeSum1E::GaussianFunction(int)>& opForZ) const
    {
    ...
    // Gaussian::LatticeSum1E: the lattice-summed overlap of every basis function with ONE caller-supplied
    ...
    cvec_t MakeOverlap(const cellphase_t& phase, const UnitCell& A,
                       const Gaussian::LatticeSum1E::GaussianFunction& g) const
    {
        const double lne=-std::log(kScreenEps());
    ...
    cvec_t MakeOverlap(const Gaussian::LatticeSum1E::GaussianFunction& g) const
    {
        const int Lg=GaussDegree(g);
        cvec_t b(size(), dcmplx(0.0));
    ...
    static int GaussDegree(const Gaussian::LatticeSum1E::GaussianFunction& g)
    {
        int Lg=0;
        for (const auto& t : g.terms) Lg=std::max(Lg, t.p.n+t.p.l+t.p.m);
        return Lg;
    }
    double GaussOverlapTerm(size_t i, const Gaussian::LatticeSum1E::GaussianFunction& g,
    ...
    // Gaussian::LatticeSum1E: the finest exponent over every component's radial primitives -- the density
    ...
    // Gaussian::LatticeSum1E: the COARSEST primitive exponent -- the diffuse end of the basis, mirroring
    ...
    // and src/BasisSet/Gaussian/tests/M_PG_BoxWalk.C is the oracle.
    ...
    //! Realises \c Gaussian::LatticeSum1E::SetStreamSymmetryOps (forwarded by the host IBS): build (or
    ...
    //! Realises \c Gaussian::LatticeSum1E::StreamFoldOrder (the fold-state cache-key input).
    ...
    // Collocate the grid density onto the multi-grid ladder (Gaussian::LatticeSum1E::CollocateDensity).  The
...
private:
    ...
    std::vector<OpTerm> BuildOpTerms(const Structure* cl,
                                     const std::function<Gaussian::LatticeSum1E::GaussianFunction(int)>& opForZ,
                                     const UnitCell* A) const
    {
        ...
        for (Atom* a : *cl)
        {
            Gaussian::LatticeSum1E::GaussianFunction g=opForZ(a->itsZ);
            const int Lg=GaussDegree(g);
