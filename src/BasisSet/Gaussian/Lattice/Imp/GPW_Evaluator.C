// File: BasisSet/Gaussian/Lattice/Imp/GPW_Evaluator.C  GPW_Evaluator implementation.
module;
#include <cassert>
#include <cmath>
#include <complex>
...
Gaussian::LatticeSum1E::cellphase_t GPW_Evaluator::CellPhase() const
{
...
std::function<ΔG_Map(const chmat_t&)> GPW_Evaluator::MakeCollocator(bool coulomb, std::shared_ptr<const PlaneWave::PW_Grid_Evaluator> grid) const
{
    ...
    const Gaussian::Periodic_Gaussian_IBS* lat = itsLat;
...
// The potential->KS bridge (the applyAdjoint realization) -- the ANALYTIC integrate-back,
...
chmat_t GPW_Evaluator::MakeLocalPPShort(const Structure* cl, const SpeciesRadialField& loc) const
{
    ...
    auto opForZ=[gauss](int Z)->Gaussian::LatticeSum1E::GaussianFunction
    {
        Gaussian::LatticeSum1E::GaussianFunction g;
