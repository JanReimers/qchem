// File: BasisSet/Gaussian/Lattice/Imp/SphericalLatticeView.C  (see the interface unit for the design)
//
...
#include <string>
#include <vector>
module qchem.BasisSet.Gaussian.Lattice.SphericalLatticeView;
...
import qchem.BasisSet.Gaussian.Point.IBS;           // Gaussian::Orbital_1E_IBS (GetAoShells -- the inner shell source)
...
{
    const Gaussian::Orbital_1E_IBS* obs=nullptr;
    for (auto ibs : const_cast<Real_BS&>(*cart).Iterate<Gaussian::Orbital_1E_IBS>()) { obs=ibs; break; }
