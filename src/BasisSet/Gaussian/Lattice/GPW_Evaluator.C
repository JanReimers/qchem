// File: BasisSet/Gaussian/Lattice/GPW_Evaluator.C  Gaussian-And-Plane-Waves orbital evaluator.
//
...
// cell's atoms and asks it the ONE high-level question Gaussian::LatticeSum1E poses -- "sum these two-centre
...
//! (\c Gaussian::LatticeSum1E) by an abstract cross-cast; supplies the lattice translation set from the cell.
...
public:
    ...
    //!              orbital block must realise \c Gaussian::LatticeSum1E (a PG Gaussian basis does).
    ...
    //! by the ANALYTIC per-pair integrate-back (\c Gaussian::LatticeSum1E::IntegratePotential) on the REL_CUTOFF
...
private:
    ...
    const Gaussian::Periodic_Gaussian_IBS* itsLat = nullptr; //!< the same block's periodic-1E capability (cross-cast)
    ...
    std::shared_ptr<const Gaussian::LatticeScreener> itsScreener;
    ...
    Gaussian::LatticeSum1E::cellphase_t CellPhase() const;
