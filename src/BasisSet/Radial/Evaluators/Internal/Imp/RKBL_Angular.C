// File: BasisSet/Radial/Evaluators/Internal/Imp/RKBL_Angular.C
module;
#include <sstream>
#include <cassert>
module qchem.BasisSet.Radial.Evaluators.Internal.RKBL_Angular;
import qchem.BasisSet.Radial.Evaluators.Internal.RelAngularIntegrals;
import qchem.Symmetry.Atom.Spherical; // SphericalSpinor::j(κ)
import qchem.Blaze;
...
rvec11_t RKB_Angular::DirectAk(const Evaluator& other) const
{
    ...
    double ja = Symmetry::Atom::SphericalSpinor::j(κ);
    double jc = Symmetry::Atom::SphericalSpinor::j(o.κ);
...
rvec11_t RKB_Angular::ExchangeAk(const Evaluator& other) const
{
    const RKB_Angular& o = dynamic_cast<const RKB_Angular&>(other);
    double ja = Symmetry::Atom::SphericalSpinor::j(κ);
    double jb = Symmetry::Atom::SphericalSpinor::j(o.κ);
