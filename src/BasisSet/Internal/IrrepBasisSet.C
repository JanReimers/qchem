...
public:
    ...
    virtual SMatrix<T> Direct  (const smat_t<T>& Dcd, const Orbital_HF_IBS<T>* bs_cd) const;
    virtual SMatrix<T> Exchange(const smat_t<T>& Dcd, const Orbital_HF_IBS<T>* bs_cd) const;
};
