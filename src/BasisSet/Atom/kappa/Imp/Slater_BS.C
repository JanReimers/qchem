...
//            for (double mj=-j;mj<=j;mj+=1.0)
        Insert(new Atom_kappa::Slater::Orbital_RKB_IBS(db,new Slater_IBS(ss.Get_es(l),l,{}),ss.Get_es(l),l));            
        // j=l+0.5 sector, kappa = -l -1 < 0
        j=l+0.5;
//        for (double mj=-j;mj<=j;mj+=1.0)
            Insert(new Atom_kappa::Slater::Orbital_RKB_IBS(db,new Slater_IBS(ss.Get_es(l),l,{}),ss.Get_es(l),-l-1));            
