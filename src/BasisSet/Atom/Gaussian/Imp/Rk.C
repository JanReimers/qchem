...
#include <cmath>
#include <vector>
module qchem.BasisSet.Atom.Gaussian.Rk;
...
Rk::rvec11_t RkEngine::Coulomb_Rk(size_t la,size_t lc) const
{
    assert(la<=LMax);
    assert(lc<=LMax);
    rvec11_t ret(0.0);
...
Rk::rvec11_t RkEngine::ExchangeRk(size_t la,size_t lb) const
{
    assert(la<=LMax);
    assert(lb<=LMax);
