...
// is valid for the basis's lifetime regardless of the density -- exactly the Projector3 caching contract.
module;
...
const Projector3<dcmplx>& Band_FT_IBS::Repulsion3C(const cFIT_CD_ABS& c) const
{
    return theCache<dcmplx>().Get(IntegralsCache_Base::I3C::Repulsion,this,&c,
        [this,&c]{ return MakeRepulsion3C(c); });
}

const Projector3<dcmplx>& Band_FT_IBS::Overlap3C(const cFIT_SF_ABS& c) const
{
