...
namespace BasisSet
{
template <class T> Fit_IBS* BasisSet<T>::CreateCDFitBasisSet(const Structure* cl, const qcMesh::MeshParams& mp) const
{
    auto dft=*Iterate<Orbital_DFT_IBS<double>>().begin();
    return dft->CreateCDFitBasisSet(cl,mp);
}
template <class T> Fit_IBS* BasisSet<T>::CreateVxcFitBasisSet(const Structure* cl, const qcMesh::MeshParams& mp) const
{
    auto dft=*Iterate<Orbital_DFT_IBS<double>>().begin();
...
template <> Fit_IBS* BasisSet<dcmplx>::CreateCDFitBasisSet (const Structure*, const qcMesh::MeshParams&) const { assert(false); return 0; }
template <> Fit_IBS* BasisSet<dcmplx>::CreateVxcFitBasisSet(const Structure*, const qcMesh::MeshParams&) const { assert(false); return 0; }
