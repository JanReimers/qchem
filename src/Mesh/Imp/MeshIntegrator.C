...
template <class T> rvec_t MeshIntegrator<T>::Normalize(const Vf& v) const
{
    ...
    {
        for (size_t i=1; i<=n; i++)
            ret[i-1]+=real(sv(i,iw)*conj(sv(i,iw))*w(rw));
        iw++;          
    }
    ...
    for (auto rw:*itsMesh)
    {
        for (size_t i=1; i<=n; i++)
            for (size_t j=i; j<=n; j++)
                ret(i-1,j-1)+=sf(i,iw)*conj(sf(j,iw))*w(rw);
        iw++;
    }
...
template <class T> vec_t<T> MeshIntegrator<T>::Overlap(const Rf& f,const Vf& v) const
{
    ...
    {
        for (size_t i=1; i<=n; i++)
            ret[i-1]+=sv(i,iw)*conj(sf(iw))*w(rw);
    ...
    for (auto rw:*itsMesh)
    {
        for (size_t fi=1; fi<=nf; fi++)
            for (size_t gi=1; gi<=ng; gi++)
                ret(fi-1,gi-1)+=conj(sf(fi,iw))*sg(gi,iw)*w(rw);
        iw++;
    }
...
template <class T> smat_t<T> MeshIntegrator<T>::Overlap3C(const Vf& f,const Sf& g) const
{
    ...
    for (size_t i=1; i<=n; i++)
        for (size_t j=i; j<=n; j++)
        {
            ...
            for (auto rw:*itsMesh)
            {
                ret(i-1,j-1)+=conj(sf(i,wi)*sf(j,wi))*sg(wi)*w(rw);
                wi++;
            }
        }
...
template <class T> smat_t<T> MeshIntegrator<T>::Repulsion(const Vf& f) const
{
    ...
    for (auto rw1=itsMesh->begin();rw1!=itsMesh->end();rw1++,iw++)
    {
        ...
        for (auto rw2=rw1+1; rw2!=itsMesh->end(); jw++,rw2++)
        {
            ...
            for (size_t i=1; i<=n; i++)
                for (size_t j=i; j<=n; j++)
                    ret(i-1,j-1)+=(conj(sf(i,iw))*sf(j,jw)+conj(sf(i,jw))*sf(j,iw))*oor;
        }
    }
    assert(!isnan(ret));
    return ret;
...
template <class T> vec_t<T> MeshIntegrator<T>::Repulsion(const Rf& h, const Vf& f) const
{
    ...
    for (auto rw1=itsMesh->begin();rw1!=itsMesh->end();rw1++,iw++)
    {
        ...
        for (auto rw2=rw1+1; rw2!=itsMesh->end(); jw++,rw2++)
        {
            ...
            assert(!std::isnan(oor));
            for (size_t i=1; i<=n; i++)
                ret[i-1]+=(sf(i,iw)*conj(sh(jw))+sf(i,jw)*conj(sh(iw)))*oor;
        }
    }
    assert(!isnan(ret));
    return ret;
        ...
        for (auto rw2=rw1+1; rw2!=itsMesh->end(); jw++,rw2++)
        {
            ...
            for (size_t i=1; i<=nf; i++)
                for (size_t j=1; j<=ng; j++)
                    ret(i-1,j-1)+=(conj(sf(i,iw))*sg(j,jw)+conj(sf(i,jw))*sg(j,iw))*oor;
        }
    }
    assert(!isnan(ret));
    return ret;
...
template <class T> smat_t<T> MeshIntegrator<T>::Repulsion3C(const Vf& f, const Sf& h) const
{
    ...
    for (auto rw1=itsMesh->begin();rw1!=itsMesh->end();rw1++,iw++)
    {
        ...
        for (auto rw2=rw1+1; rw2!=itsMesh->end(); jw++,rw2++)
        {
            ...
            for (size_t i=1; i<=n; i++)
                for (size_t j=i; j<=n; j++)
                    ret(i-1,j-1)+=(conj(sf(i,iw)*sf(j,iw))*sh(jw) + conj(sf(i,jw)*sf(j,jw))*sh(iw))*oor;
        }
    }
    return ret;
}
...
template <class T> smat_t<T> MeshIntegrator<T>::Inv_r1(const Vf& f) const
{
    ...
    for (size_t i=1; i<=n; i++)
        for (size_t j=i; j<=n; j++)
        {
            ...
            for (auto rw:*itsMesh)
            {
                if (norm(r(rw))!=0) ret(i-1,j-1)+=conj(sf(i,wi))*sf(j,wi)*w(rw)/norm(r(rw));
                wi++;
            }
        }
    return ret;
...
template <class T> smat_t<T> MeshIntegrator<T>::Inv_r2(const Vf& f) const
{
    ...
    for (size_t i=1; i<=n; i++)
        for (size_t j=i; j<=n; j++)
        {
            ...
            {
                double mr=norm(r(rw));
                if (mr!=0) ret(i-1,j-1)+=conj(sf(i,wi))*sf(j,wi)*w(rw)/(mr*mr);
                wi++;
            }
        }
    return ret;
...
template <class T> smat_t<T> MeshIntegrator<T>::Grad2(const Vf& f) const
{
    ...
    for (size_t i=1; i<=n; i++)
        for (size_t j=i; j<=n; j++)
        {
            ...
            for (auto rw:*itsMesh)
            {
                 ret(i-1,j-1)+=conj(sf(i,wi))*sf(j,wi)*w(rw);
                 wi++;
            }
        }
    return ret;
...
template <class T> mat_t<T> MeshIntegrator<T>::Grada_b(const Vf& a,const Vf& b) const
{
    ...
    for (size_t i=1; i<=na; i++)
        for (size_t j=1; j<=nb; j++)
        {
            ...
            for (auto rw:*itsMesh)
            {
                ret(i-1,j-1)+=conj(norm(sa(i,wi)))*sb(j,wi)*w(rw);
                wi++;
            }
        }
    return ret;
...
template <class T> mat_t<T> MeshIntegrator<T>::a_Gradb(const Vf& a,const Vf& b) const
{
    ...
    for (size_t i=1; i<=na; i++)
        for (size_t j=1; j<=nb; j++)
        {
            ...
            for (auto rw:*itsMesh)
            {
                ret(i-1,j-1)+=conj(sa(i,wi))*norm(sb(j,wi))*w(rw);
                wi++;
            }
        }
    return ret;
