...
{
// EVERY Hamiltonian is built FOR an imposed spin subgroup (V1.37 step 3): the group is a ctor argument
// stored by rHamiltonianImp, and the XC terms receive it -- one term class per operator, never a
// polarized TYPE beside an unpolarized one.  (The eleven Ham_*_U / Ham_*_P classes this replaced differed
// only in which of two term types they Added.)  The Dirac Hamiltonians are the exception that proves the
// rule: their (κ,m_j) blocks carry spin INSIDE the double group, the tree has no folded form for them, so
// they answer Polarized regardless of what was asked (doc/BasisSetTaxonomyPlan.md §1.4, last row).
...
{
public:
    Ham_1E(const st_t& st, SpinGroup g);
};

//
//  Hartree-Fock: exact Coulomb + same-spin exchange (RHF on a folded doublet, UHF on Up/Down blocks).
//
class Ham_HF : public virtual rHamiltonian, private rHamiltonianImp
{
public:
    Ham_HF(const st_t& st, SpinGroup g);
};

//
//  DFT with a fitted Hartree potential and ONE fitted XC term over a SUM of functionals (a full LDA is
//  Dirac exchange + VWN5 correlation, summed so the fit runs once -- the molecular twin of the periodic
//  one-gather rule).  The correlation energy is the functional's own eps_c, not the exchange virial.
//
class Ham_DFT : public virtual rHamiltonian, private rHamiltonianImp
{
public:
    //! Slater-Dirac exchange alone, scaled by \a alpha_ex (X\f$\alpha\f$).
    Ham_DFT(const st_t& st, double alpha_ex, const qcMesh::MeshParams&, const rbs_t* bs, SpinGroup g);
    //! The general form: any list of functionals (owned), summed into one XC term.
    Ham_DFT(const st_t& st, std::vector<std::shared_ptr<ExFunctional>> parts,
            const qcMesh::MeshParams&, const rbs_t* bs, SpinGroup g);
};
...
//! \a g is the imposed spin subgroup; everything but the XC term is spin-agnostic.
...
//
//  Dirac-Hartree-Fock -- always Polarized (see the note at the top of this namespace).
//
...
class Ham_DHF : public virtual rHamiltonian, private rHamiltonianImp
{
public:
    Ham_DHF(const st_t& st);
};
