...
| **MK** | ⛔ **A REAL MULTI-k DEFECT, FOUND 2026-09-09 BY RUNNING A DISABLED TEST: THE IBZ WEIGHTS DO NOT SUM TO 1.**  `GPW_SCF.DISABLED_SiliconMultiKPlumbing` (2×1×1 MP mesh, Si) reports **charge = 12 against 8 valence electrons**, and its own run banner says why: **`[IBZ] 2 k-points -> 2 irreducible; Σw=1.5`**.  8 × 1.5 = 12, exactly.  ★ **THIS IS NOT A STALE ENERGY ANCHOR** (the category the user rightly flagged for these tests): 8 valence electrons is physics and cannot go stale, and the number is printed by the code itself.  The energy assertion in the same test is un-judgeable until the charge is right — an SCF that converged carrying 12 electrons has a meaningless total.  ⇒ **Weights are `starSize/Ntot`, so Σw>1 means the stars OVERLAP** — Σ starSize = 3 on a 2-point grid — i.e. `FoldGrid` counts a point in more than one star.  Both k here (Γ and the zone boundary) are TRIM, which is the standing trap (`feedback_complex_type_vs_value`: *every k on a 2×2×2 mesh is TRIM*).  ✅ **`IBZMesh::WeightSum()`'s own doc has always said "should be 1" — it just said it to nobody.**  Now ENFORCED at construction (`BZReduction.C`), throwing with both numbers: the whole suite stays green (844/844), so no ENABLED test was running on bad weights, and the defect is confined to the multi-k path. | ▶ Fix `FoldGrid`'s star counting for self-paired (TRIM) grid points, then re-judge the energy anchor — not before.  ⚠ This sits directly under item **KP**: multi-k is the axis every other code parallelises over, and it currently cannot even conserve charge on a 2-point mesh. | this row + **KP** |
