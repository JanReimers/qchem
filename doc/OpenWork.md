...
> # ★★★ THE QUEUED PROGRAMME (agreed 2026-09-09) — WORK IT IN THIS ORDER
>
...
> the reciprocal op \f$U\f$ to the grid INDEX vector: the index action is the CONJUGATED
> \f$M=DUD^{-1}\f$, which equals \f$U\f$ only on an ISOTROPIC mesh, and on an anisotropic one the
> mod-\f$N\f$ wrap turned a non-symmetry into a non-bijection, so the "orbits" overlapped.  Ops whose
> \f$M\f$ is non-integral (or that miss the shift) are now skipped wholesale — the fold is under the
...
> speed.  Sampling four: **two passed and were cheap** (12 s, 23 s), one looked like a stale anchor, one was
> **KP-0**.  ⇒ Each re-enable either banks a gate cheaply or tells you something.
> ⚠ **Judge a failing ENERGY anchor before believing it** (user, 2026-09-09: *"some of those tests may have
> invalid energy anchors"*) — but a failing CHARGE or count is physics and cannot go stale.  KP-0 is the
> worked example of telling the two apart.
>
> ▶ **BATCH 3 (2026-09-09) — the two filed rows closed on user policy, plus a test cull.**
> **R2.5b**: all nine remaining `exit(-1)` are throws; the tree is `exit()`-free outside tests.  ★ The
> durable part is the POLICY: *"we really don't have a proper error handling policy … throwing exceptions
> is the best interim solution.  The key is that they are all easy to search and find (just hunt for the
> `throw` token), so if/when we do architect a proper error/warning framework we can locate all error
> points and react accordingly."*  ⇒ **`throw` is a MARKER**, and `cerr`+`exit` is never acceptable in
> library code because it is unsearchable as a class.  **V1.20b**: `GMap` promoted out of `Internal` —
> honesty about its reach, not architecture; the real question (a return type that varies with the basis
> without naming the representation) stays open and is **D2** from the other end.
> ★ **AND 18 DISABLED TESTS ARE DELETED** (user: printf sweeps *"are most likely left overs from a
> debugging, optimization or memory leak hunt campaign … any new campaigns of this sort will just create
> new ones tailored to the current needs"*).  Criterion applied mechanically: **zero `EXPECT_`/`ASSERT_`
> in the body**.  Disabled count 42 → 24; enabled 850/850 unchanged.  ⚠ ONE zero-assert test was KEPT and
...
> unit tests (41 ms) replace `PolarizedRunKeepsItsSpin` (217 s); **`ctest -j8` 207 s → 71 s wall**, 862 enabled.  ⇒ **PHASE 2
> (the harness collapse onto `SolidCalculation` + `BravaisCell` + `materials.json`) IS NEXT.**  Two holes it already
...
> *(The old "one next action: size the Becke grid" banner is retired as the entry point — it is
> item 1 of the table below, and the queue above supersedes it.)*
>
