...
> starts at step 1 and does not shop around the table below** — the table is the reference, this is the
> queue.
>
> ### 1. ⛔ FIX `KP-0` FIRST — it is a BUG, not cleanup
> The IBZ k-point weights sum to 1.5 on a 2×1×1 mesh, so a multi-k run carries **12 electrons where it
> should carry 8**.  Fix `FoldGrid`'s star counting for self-paired (TRIM) grid points, then re-judge
> `SiliconMultiKPlumbing`'s energy anchor — *not before*, because an SCF converged on 12 electrons has a
> meaningless total.  ▶ Everything k-related is downstream of this, **including +U on a supercell**, which
> is the north-star deck.
>
...
| **KP-0** | ⛔ **BLOCKS KP, AND IT IS A BUG NOT A CLEANUP — the IBZ weights do not sum to 1** (found 2026-09-09 by running a disabled test).  *Renamed from the one-session-old id "MK": two rows in this table both meaning "k-points", with two opaque two-letter codes, is how a tracker becomes unreadable — and it is a PREREQUISITE of KP, not a sibling.  **You cannot parallelise over k-points while the k-point weights are wrong.*** | ⛔ **A REAL MULTI-k DEFECT, FOUND 2026-09-09 BY RUNNING A DISABLED TEST: THE IBZ WEIGHTS DO NOT SUM TO 1.**  `GPW_SCF.DISABLED_SiliconMultiKPlumbing` (2×1×1 MP mesh, Si) reports **charge = 12 against 8 valence electrons**, and its own run banner says why: **`[IBZ] 2 k-points -> 2 irreducible; Σw=1.5`**.  8 × 1.5 = 12, exactly.  ★ **THIS IS NOT A STALE ENERGY ANCHOR** (the category the user rightly flagged for these tests): 8 valence electrons is physics and cannot go stale, and the number is printed by the code itself.  The energy assertion in the same test is un-judgeable until the charge is right — an SCF that converged carrying 12 electrons has a meaningless total.  ⇒ **Weights are `starSize/Ntot`, so Σw>1 means the stars OVERLAP** — Σ starSize = 3 on a 2-point grid — i.e. `FoldGrid` counts a point in more than one star.  Both k here (Γ and the zone boundary) are TRIM, which is the standing trap (`feedback_complex_type_vs_value`: *every k on a 2×2×2 mesh is TRIM*).  ✅ **`IBZMesh::WeightSum()`'s own doc has always said "should be 1" — it just said it to nobody.**  Now ENFORCED at construction (`BZReduction.C`), throwing with both numbers: the whole suite stays green (844/844), so no ENABLED test was running on bad weights, and the defect is confined to the multi-k path. | ▶ **DO THIS FIRST of anything k-related, including +U on a supercell.**  Fix `FoldGrid`'s star counting for self-paired (TRIM) grid points; then re-judge `SiliconMultiKPlumbing`'s ENERGY anchor, which is un-judgeable until the charge is 8. | this row + **KP** |
