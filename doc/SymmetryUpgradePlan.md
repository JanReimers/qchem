...
**T2 REMAINING (needs its own design pass before code): the XC wiring** — build the
periodic Becke mesh site-adapted (per-atom-orbit grids generated by the ops), star-average ρ
on it per iteration, fold the quadrature under the §3 policy, PROVE the E/H adjoint pairing
under the fold (the §9 route-(b)-class variational pin — energy folds at reps but the KS
matrix needs every point unless the pairing is shown exact), then the Becke+IBZ gate that
retires the `reduceBZ` uniform-raster carve-out (`ResolveXCMesh`).
