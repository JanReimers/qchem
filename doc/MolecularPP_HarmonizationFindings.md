...
**But the polarized/hard path is still blocked — and the blocker is NOT in the PP code.** It is a numerical
robustness bug in the **SCF-accelerator / linear-algebra layer**, hit for ill-conditioned polarized cases:
`SCFAcceleratorDIIS::UseFD` → `LASolverCholesky::Transform` → `transform_impl` → **`make_hermitian`**
(`src/LASolver/Internal/Imp/LASolverLapack.C:23`) wraps a Cholesky-transformed matrix in a Blaze
`SymmetricMatrix`; the matrix contains **NaN/Inf** (from a near-singular overlap / breakdown upstream), so the
symmetry check fails → *"Invalid setup of symmetric matrix"*. It is basis-and-accelerator-independent (sipp,
szvgth, dzvpgth all throw; DIIS **and** GDM route through the same shared `Transform`) and reproduces for
polarized DFT with small bases too (cf. the old "Slater N5" note) — so it is orthogonal to PP. The real fix is
a robust orthonormalization (canonical/level-shifted, dropping the near-null-space) in the accelerator/LASolver,
which is the accelerator owner's domain. Until then, `Ham_PP`'s polarized path is correct-by-construction (it
reuses the polarized DFT terms, which are tested) but cannot run a hard open-shell PP molecule end-to-end.
