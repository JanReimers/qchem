...
## Piece 2 — `Ham_DFTcorr_P` + `FittedVcorrPol` (carry both channels; U = collapse)
...
| **B2** | `FittedVcorrPol` + `Ham_DFTcorr_P` + Factory un-gate (Piece 2). Seed fallback. | polarized LDA water converges to the **unpolarized LDA anchor** to ~1e-11 (mirrors the `M_DFT.WaterPolarizedSAD` Xα guard); `_U` path byte-identical |
