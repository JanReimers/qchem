...
- **The `MakeFourierDensity(D)` density-matrix leak (headline SRP item for this review).**
  ...
  2. **The basis functions must not leak into the density.** The molecular density presents a clean neutral face
     (`rho(r)`, a `ScalarFunction`); its consumers (fitters, KS terms) see `rho`, never `{φ}` or `D`. The
     reciprocal-space neutral bridge already exists and is the exact analogue of `rho(r)`-on-a-mesh: the
     **rho-tilde `ΔG_Map`** — and `Band_FT_IBS::Repulsion(const ΔG_Map&)` already consumes it cleanly. So it is
     **only the `D → rho-tilde` PRODUCTION** that is misplaced (a basis method that swallows `D`).
  - **Target:** the `FourierDensity` should expose **rho-tilde as its neutral face** (mirroring molecular
    `rho(r)`), and the basis assembly should speak **only rho-tilde** — neither `D` nor `{G}`/plane-wave
    functions crossing the boundary. **The tension to resolve** (this is the actual design work): `rho-tilde =
    (1/Ω) Σ_{Gᵢ−Gⱼ=Δm} Dᵢⱼ` genuinely needs BOTH `D` (density) and the `{G}`-difference structure (basis) — the
    same bind as molecular `rho(r)=Σφᵢ Dᵢⱼ φⱼ`. Molecular resolved it by having the *density* own its basis
    handle and do the contraction (`IrrepCD::operator()` evaluates `(*itsBasisSet)(r)`), keeping the *assembly
    interface* clean. Decide the plane-wave placement the same way — put the `D→rho-tilde` contraction on the
    density (given a neutral `{G}`-structure seam), not as `Band_FT_IBS::MakeFourierDensity(D)`. **Keep** the
    *other* overload `MakeFourierDensity(const Structure*, formFactor)` — it takes a structure + form factor (no
    `D`), is the density-analogue of `MakeLocalPotential` (the SAD seed), and does not leak.
